      ...
      pRetry(
        async () => {
          try {
            ...
            const abort = pRetry as unknown as { AbortError: new (e: Error) => Error };
            throw new abort.AbortError(
      ...
      const result = await pRetry(
        () => processor(item, index),
        {
          ...
          onFailedAttempt: (error) => {
            if (!isRateLimitError(error)) {
              const abort2 = pRetry as unknown as { AbortError: new (e: Error) => Error };
            throw new abort2.AbortError(
                error instanceof Error ? error : new Error(String(error))
              );
            }
          },
