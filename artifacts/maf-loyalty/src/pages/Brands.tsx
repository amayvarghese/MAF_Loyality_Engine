  ...
  return (
    <AppLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-4xl font-display font-bold text-white mb-2">MAF Brand Directory</h1>
          ...
          {brands?.map(brand => (
            <Card key={brand.id} className="group cursor-pointer hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="p-8">
                <div className="w-16 h-16 rounded-2xl mb-6 flex items-center justify-center" style={{ backgroundColor: `${brand.logoColor}20` || 'rgba(255,255,255,0.1)' }}>
                  <Building2 className="w-8 h-8" style={{ color: brand.logoColor || '#fff' }} />
                </div>
                <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-primary transition-colors">{brand.name}</h3>
                ...
                <div className="mt-6 pt-6 border-t border-white/10 flex items-center text-primary font-medium opacity-0 group-hover:opacity-100 transition-opacity translate-x-[-10px] group-hover:translate-x-0 duration-300">
